...
Based on Marko Heijnen's wp-db-driver:
...
class wkdb
{
    ...
    protected function set_driver() {
        ...
        if ( ! call_user_func( array( $driver, 'is_supported' ) ) ) {
            $this->bail( __( "
				<h1>The pdo extends is not supported</h1>.
				<p>WordPress requires the pdo_mysql extension to talk to your database.</p>
...
class wkpdo {
     ...
     * Perform a MySQL database query, using current database connection.
     * @param string $query Database query
     * @return PDOStatement|false Number of rows affected/selected or false on error
     */
    public function query( $query ) {
 ...
 * @return array
 */
function convert_pdo_placeholders($sql, $params) {
    if (count($params) > 0) {
        ...
        $contents = array(); // collect content inside quotes
        $i = 0;
        $sql = preg_replace_callback('/(("[^"\\\\]*(?:\\\\.[^"\\\\]*)*")|(`[^`\\\\]*(?:\\\\.[^`\\\\]*)*`)|(\'[^\'\\\\]*(?:\\\\.[^\'\\\\]*)*\'))/s', function($matches) use(&$i, &$contents) {
            $replace = '{{' . $i . '}}';
        ...
        if (is_int($first_key)) { // unnamed placeholders
            ...
            $sql = preg_replace_callback('/([:][a-zA-Z]+[a-zA-Z0-9_]+)/', function($matches) use($params) {
...
$wpdb = new wkdb(DB_USER, DB_PASSWORD, DB_NAME, DB_HOST);
